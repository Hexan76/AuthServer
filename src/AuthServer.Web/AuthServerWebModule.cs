...
using AuthServer.Web.HealthChecks;
...
public class AuthServerWebModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(AuthServerDomainModule).Assembly,
                typeof(AuthServerDomainSharedModule).Assembly,
                typeof(AuthServerApplicationModule).Assembly,
                typeof(AuthServerApplicationContractsModule).Assembly,
                typeof(AuthServerWebModule).Assembly
            );
        });
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<AuthServerWebModule>();

            if (hostingEnvironment.IsDevelopment())
            {
                options.FileSets.ReplaceEmbeddedByPhysical<AuthServerDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AuthServer.Domain.Shared", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AuthServerDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AuthServer.Domain", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AuthServerApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AuthServer.Application.Contracts", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AuthServerApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AuthServer.Application", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AuthServerHttpApiModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}AuthServer.HttpApi", Path.DirectorySeparatorChar)));
                //options.FileSets.ReplaceEmbeddedByPhysical<AuthServerWebModule>(hostingEnvironment.ContentRootPath);
            }
        });
    }
    ...
    private void ConfigureNavigationServices()
    {
        ...
        //{
        //    options.Contributors.Add(new AuthServerToolbarContributor());
        //});
    }
